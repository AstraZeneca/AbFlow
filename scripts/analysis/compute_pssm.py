...
def evaluate_dataset_with_pssm(target: str, results_dir: str, compute_auc: bool=False):
    ...
    csv_path = os.path.join(results_dir, f"{GLOBAL_DATA}_{target_name}_pssm_correlation_{EPOCH_NUM}.csv")
...
if __name__ == "__main__":
    ...
    all_csv = os.path.join(RESULTS_DIR, f"{GLOBAL_DATA}_all_datasets_pssm_summary_{EPOCH_NUM}.csv")
    ...
    spearman_csv   = os.path.join(RESULTS_DIR, f"{GLOBAL_DATA}_all_datasets_spearman_summary_{EPOCH_NUM}.csv")
    kendall_csv    = os.path.join(RESULTS_DIR, f"{GLOBAL_DATA}_all_datasets_kendall_summary_{EPOCH_NUM}.csv")
