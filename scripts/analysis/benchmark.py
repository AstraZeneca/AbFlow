...
experiment_name = 'abflow_may17_2dmask_CDR3ONLY_Fixed_numAtomAndMasking_separateEmbds_sabdab_sequence_backbone'
epoch_num = 'epoch=114' #'epoch=199'
...
def evaluate_mutated_pdb(config, target, pdb_file, parent_info, mutated_info, scheme, results_dir, num_designs, batch_size, seed, epoch_num=0, compute_auc=False):
    ...
    try:
        ...
        save_path = f"{results_dir}/z108__{target}_{file_prefix}_{epoch_num}_cnum_{config['network']['n_cycle']}_{config['shared']['design_mode']}_numdesg{num_designs}.png"
...
# Save results to CSV
os.makedirs(results_dir, exist_ok=True)
output_path = os.path.join(results_dir, f"z108__{file_prefix}_rabd_metrics_{epoch_num}_cycle_num_{config['network']['n_cycle']}_Ver2_{config['shared']['design_mode']}_{num_designs}.csv")
...
dataset1 = ['absci_her2_zs', 'absci_her2_sc', 'nature_hel', 'nature_il7', 'nature_her2', 'napi2b', 'sonic'] #, 'tweak'
