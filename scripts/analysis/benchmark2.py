...
def evaluate_mutated_pdb(config, target, pdb_file, parent_info, mutated_info, scheme, results_dir, num_designs, batch_size, seed, epoch_num=0, compute_auc=False):
    ...
    try:
        ...
        save_path = f"{results_dir}/z109__{target}_{file_prefix}_{epoch_num}_cnum_{config['network']['n_cycle']}_{config['shared']['design_mode']}_numdesg{num_designs}.png"
...
def evaluate_mutated_pdb_with_blosum(*args, **kwargs):
    ...
    # 4) correlations + significance stars
    ...
    # 5) scatter with legend text only
    plt.figure(figsize=(6,6))
    ...
    plt.xlabel("BLOSUM45 distance", fontsize=14, fontweight='bold')
    plt.ylabel(r"$-\log(K_D)$",      fontsize=14, fontweight='bold')
    plt.grid(linestyle='--', alpha=0.5)
    plt.tight_layout()

    blosum_path = f"{results_dir}/blosum{BLOSUM_NUM}_vs_{target}_{file_prefix}_{epoch_num}.png"
...
for d2_source in dataset2_pdb_source:
    ...
    for complex_name in complex_list:
        ...
        evaluate_mutated_pdb_with_blosum(config=config,
                                        target=complex_name,
                                        pdb_file=pdb_file,
