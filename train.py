...
def setup_model(config, checkpoint_path=None, load_optimizer=False):
    ...
    if checkpoint_path is not None and load_optimizer:
        ...
        model_instance = AbFlow.load_from_checkpoint(checkpoint_path=checkpoint_path, network=network_instance, **config['model'])
...
def train(config, data_loader, save_weights=True):
    ...
    # Logger
    today = date.today()
    logger = WandbLogger(project=config["project_name"], name=config["model_name"], log_model=False) 
